"""Review-only probes: assignment rebinding and expiry while waiting for SQLite."""
import asyncio
import json
...
async def main():
    ...
    results = []
    for case in ('rebind_attempt', 'rebind_coordinator', 'lease_expires_waiting_for_lock'):
